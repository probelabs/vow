...
function backupFile(filePath) {
  ...
  const backupDir = path.join(repoRoot, BACKUP_DIR);
  ...
  try {
    ...
    console.error(`Failed to backup ${filePath}: ${e.message}`);
    return null;
  }
}
...
`;
  ...
  // Backup existing Vow hook if it exists
  let backupPath = null;
  if (fs.existsSync(vowHookPath) && !dryRun) {
    backupPath = backupFile(vowHookPath);
  }
  ...
  if (dryRun) {
    return {
      ...
      backup: backupPath ? 'Would backup existing Vow hook' : null,
  ...
  try {
    ...
    if (fs.existsSync(mainHookPath)) {
      ...
      // Backup main hook before modifying
      mainBackupPath = backupFile(mainHookPath);
    } else {
...
function installHuskyHook(detection, options = {}) {
  ...
  if (fs.existsSync(hookPath)) {
    ...
    if (!dryRun) {
      backupPath = backupFile(hookPath);
    }
  }
...
`;
  ...
  if (dryRun) {
    return {
      ...
      wouldWrite: hookPath,
      content: hookContent,
      backup: backupPath ? 'Would backup existing hook' : null
    };
  }
...
function installHuskyV4(detection, options = {}) {
  ...
  try {
    ...
    // Backup package.json
    const backupPath = backupFile(packagePath);
...
function installClaudeCode(detection, options = {}) {
  ...
  if (fs.existsSync(settingsPath)) {
    try {
      ...
      if ((settings.gitCommitHook || settings.gitPreCommitHook || '').includes('vow')) {
        if (!force) {
          return {
            success: false,
            error: `Vow is already configured in ${settingsFileName}`,
  ...
  // Add Vow configuration
  settings.gitCommitHook = VOW_COMMAND;
  ...
  if (dryRun) {
    return {
      ...
      backup: backupPath ? 'Would backup existing settings' : null,
      scope: scope
    };
  }
