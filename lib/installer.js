...
function installClaudeCode(detection, options = {}) {
  ...
  // Add permission for vow consent command
  const vowConsentPermission = 'Bash(npx -y @probelabs/vow@latest consent:*)';
  if (!settings.permissions.allow.includes(vowConsentPermission)) {
    settings.permissions.allow.push(vowConsentPermission);
  }
