...
function installClaudeCode(detection, options = {}) {
  ...
  // Gate sub-agent runs *before* they start to avoid SubagentStop recursion
  const taskPreHook = {
    "matcher": "Task",
    "hooks": [
      {
        "type": "command",
        "command": `${VOW_COMMAND} --hook --hook-type=PreToolUse`
      }
    ]
  };
  ...
  const subagentStopHook = {
    "hooks": [
      {
        "type": "command",
        "command": `${VOW_COMMAND} --hook --hook-type=SubagentStop`
      }
    ]
  };
  ...
  // Add the Task matcher PreToolUse hook if missing
  const existingTaskPreHook = settings.hooks.PreToolUse.find(h =>
    h.matcher === "Task" &&
    h.hooks &&
    h.hooks.some(hook => hook.command && hook.command.includes('vow'))
  );
  if (!existingTaskPreHook || force) {
    if (existingTaskPreHook && force) {
      const idx = settings.hooks.PreToolUse.indexOf(existingTaskPreHook);
      settings.hooks.PreToolUse.splice(idx, 1);
    }
    settings.hooks.PreToolUse.push(taskPreHook);
  }
  ...
  const existingSubagentStopHook = settings.hooks.SubagentStop.find(h =>
    h.hooks &&
    h.hooks.some(hook => hook.command && hook.command.includes('vow'))
  );
  
  if (!existingSubagentStopHook || force) {
    if (existingSubagentStopHook && force) {
      // Remove existing hook first
      const index = settings.hooks.SubagentStop.indexOf(existingSubagentStopHook);
      settings.hooks.SubagentStop.splice(index, 1);
    }
    settings.hooks.SubagentStop.push(subagentStopHook);
  }
