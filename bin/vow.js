...
function checkVow(options = {}) {
  ...
  if (!consentExists) {
    ...
    if (hookMode) {
      const minimalMessage = `Stop and fully re-evaluate your last response and task against <vow-rules>. Take time for separate thinking. If you do not remember <vow-rules> word by word, or have never seen them, first read using: \`npx -y @probelabs/vow@latest rules --ai\` While checking rules, report only the ones which fail, if none you can say "✓ All rules satisfied" (no other text). <vow-code>${validationCode}</vow-code>`;
